...
public class TensorFlowClassifier implements Classifier {
    ...
    //must be a classification percentnage greater than this
   ...
   //given a model, its label file, and its metadata
    ...
    @Override
    //public Classification recognize(final float[] pixels) {//byfei
    public Classification recognize(final float[] unlabelData) {
        //using the interface
        //give it the input name, raw pixels from the drawing,
        //input size
        //tfHelper.feed(inputName, pixels, 1, inputSize, inputSize, 1);//byfei
        tfHelper.feed(inputName, unlabelData, 1, 1, inputSize);
        ...
        for (int i = 0; i < output.length-1; ++i) {
            ...
            if (output[i] > THRESHOLD ) {///&& output[i] > ans.getConf()
                ans.update(output[i], "1");
            }
            else
                ans.update(output[i], "0");
        }
